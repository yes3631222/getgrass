...
async def task_single(user):
    ...
    agent = redis_util.get(redis_prefix_agent + to_md5(proxy_ip))
    logger.info(f"当前模拟设备id:{device_id}, 固定浏览器agent:{agent}")
    ...
    while True:
        try:
            async with proxy_connect(uri, ssl=ssl_context, extra_headers={"User-Agent": agent}, proxy=myproxy,
                ...
                logger.info(f"{myproxy.proxy_host}老板， GRASS平台auth认证响应成功：{auth_response}")
                ...
                while True:
                    ...
                    logger.info(f"{myproxy.proxy_host} 老板，GRASS平台心跳碰撞响应成功：{pong_response}")
            ...
            logger.error(f"老板，代理连接失败，准备重连 {myproxy.proxy_host}: {myproxy.proxy_port} ，异常信息：{e}")
